...
`file://` Uri, `importBundledAsset` for the asset). The catalog never writes
dictionary rows itself.
...
of stacking a second copy. Installed state is read from `DictionaryMeta`, so the
row says **Installed** from the same truth **Manage Dictionaries** shows.
...
| Test | What it pins |
|---|---|
| `DictionaryCatalogTest` | the shipped asset lists JMdict/KANJIDIC/pitch; every network row is pinned to a dated release with size + hash; the bundled row's pin matches the committed zip; installed-state matching; strict parsing |
